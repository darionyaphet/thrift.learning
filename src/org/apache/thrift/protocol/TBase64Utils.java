...
class TBase64Utils {
	 ...
	 * dst at offset dstOff. len must be 1, 2, or 3. dst must have at least
	 * len+1 bytes of space at dstOff. src and dst should not be the same
	 * object. This method does no validation of the input values in the
	...
	static final void encode(byte[] src, int srcOff, int len, byte[] dst, int dstOff) {
		...
		if (len == 3) {
			...
			dst[dstOff + 2] = (byte) ENCODE_TABLE
					.charAt(((src[srcOff + 1] << 2) & 0x3C) | ((src[srcOff + 2] >> 6) & 0x03));
	 ...
	 * dst at offset dstOff. len must be 2, 3, or 4. dst must have at least
	 * len-1 bytes of space at dstOff. src and dst may be the same object as
	 * long as dstoff <= srcOff. This method does no validation of the input
	...
	static final void decode(byte[] src, int srcOff, int len, byte[] dst, int dstOff) {
		...
		if (len > 2) {
			dst[dstOff + 1] = (byte) (((DECODE_TABLE[src[srcOff + 1] & 0x0FF] << 4) & 0xF0)
					| (DECODE_TABLE[src[srcOff + 2] & 0x0FF] >> 2));
			if (len > 3) {
				dst[dstOff + 2] = (byte) (((DECODE_TABLE[src[srcOff + 2] & 0x0FF] << 6) & 0xC0)
