...
abstract class TSaslTransport extends TTransport {
	...
	/**
	 * Whether or not we should wrap/unwrap reads/writes. Determined by whether
	 * or not a QOP is negotiated during the SASL handshake.
	 */
	 ...
	 *
	 * @param saslClient
	 *            The <code>SaslClient</code> which this transport will use for
	 *            SASL negotiation.
	 * @param underlyingTransport
	 *            The thrift transport which this transport is wrapping.
	 */
	 ...
	 *             transport, or if a status code of BAD or ERROR is
	 *             encountered.
	 */
	 ...
	 * and string message, and then throw a TTransportException with the given
	 * message.
	 *
	 * @param status
	 ...
	 * @return always throws TTransportException but declares return type to
	 *         allow throw sendAndThrowMessage(...) to inform compiler control
	 *         flow
	 */
	...
	/**
	 * Implemented by subclasses to start the Thrift SASL handshake process.
	 * When this method completes, the <code>SaslParticipant</code> in this
	 * class is assumed to be initialized.
	 *
	 * @throws TTransportException
	...
	public void open() throws TTransportException {
		/*
		 * readSaslHeader is used to tag whether the SASL header has been read
		 * properly. If there is a problem in reading the header, there might
		 * not be any data in the stream, possibly a TCP health check from load
		 * balancer.
		 */
		...
		try {
			...
			while (!sasl.isComplete()) {
				...
				// If we are the client, and the server indicates COMPLETE, we
				// don't need to
			...
			// with ANONYMOUS auth, for example, where we send an initial
			// response
			...
			/*
			 * If there is no-data or no-sasl header in the stream, throw a
			 * different type of exception so we can handle this scenario
			 * differently.
			 */
	 ...
	 *
	 * @return The <code>SaslClient</code>, or <code>null</code> if this
	 *         transport is backed by a <code>SaslServer</code>.
	 */
	 ...
	 *
	 * @return The <code>SaslServer</code>, or <code>null</code> if this
	 *         transport is backed by a <code>SaslClient</code>.
	 */
	 ...
	 *
	 * @throws TTransportException
	 *             Thrown if there's an error reading from the underlying
	 *             transport.
	 * @throws SaslException
	 ...
	 * <code>SaslClient</code> classes, which share a lot of their interface,
	 * but unfortunately don't share a common superclass.
	 */
