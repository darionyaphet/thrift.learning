 ...
 *
 * This class offers two implementations of the HTTP transport. One uses
 * HttpURLConnection instances, the other HttpClient from Apache Http
 * Components. The chosen implementation depends on the constructor used to
 * create the THttpClient instance. Using the THttpClient(String url)
 * constructor or passing null as the HttpClient to THttpClient(String url,
 * HttpClient client) will create an instance which will use HttpURLConnection.
 *
 * When using HttpClient, the following configuration leads to 5-15% better
 * performance than the HttpURLConnection implementation:
 *
 * http.protocol.version=HttpVersion.HTTP_1_1
 * http.protocol.content-charset=UTF-8 http.protocol.expect-continue=false
 * http.connection.stalecheck=false
 *
 * Also note that under high load, the HttpURLConnection implementation may
 * exhaust the open file descriptor limit.
 *
 * @see <a href="https://issues.apache.org/jira/browse/THRIFT-970">THRIFT-970
 *      </a>
 */
...
public class THttpClient extends TTransport {
	 ...
	 * httpcore that doesn't have a consume.
	 */
	...
	private void flushUsingHttpClient() throws TTransportException {
		...
		try {
			...
			// Read the responses into a byte array so we can release the
			// connection
			// early. This implies that the whole content will have to be read
			// in
			// memory, and that momentarily we might use up twice the memory
			// (while the
			// thrift struct is being read up the chain).
			// Proceeding differently might lead to exhaustion of connections
			// and thus
			// to app failure.
			...
			// Abort method so the connection gets released back to the
			// connection manager
			if (null != post) {
				post.abort();
			}
			...
			if (null != is) {
				// Close the entity's input stream, this will release the
